...
- **注釈は行単位・範囲でも付けられる**。3 種すべてキーは `"h012"`（ハンク
  全体）/ `"h012:58"`（特定行、58 = 新側 RIGHT 行番号）/ `"h012:58-63"`
  （複数行の範囲、両端とも新側 RIGHT 行）の 3 形式。行/範囲キーは終了行の
  直下にインライン表示。「どこの話か」が明確になるので可能な限り行/範囲キーで。
  行番号は extract に実在する新側行のみ、範囲は両端とも実在すること（無ければ
  ハンク単位にフォールバック+警告）。
