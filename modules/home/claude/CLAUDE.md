...
- 解説・説明を求められた場合は、回答内容を Markdown ファイルとして `/tmp` に出力し、`arto` コマンドで開くこと
  - `which arto` でインストールを確認し、存在しない場合はターミナルへの通常出力にフォールバックする
  - ファイル名は内容を端的に表す英語のケバブケースとする（例: `/tmp/nix-flake-overview.md`）
