...
- 解説・説明を求められた場合は、まず出力方法を依頼者に質問すること。選択肢は次の2つ：
  - `html` スキル：HTML の説明ドキュメントを作成し、`herdr-browser` plugin でプレビュー表示（`HERDR_ENV=1` のときのみ）する。ローカルに残る
  - Claude のアーティファクト：claude.ai 上でホストされるページとして出力する
  - 選択後はそれぞれの手順に従う。`html` スキルは herdr セッション外（`HERDR_ENV` 未設定）ではスキルの指示に従い保存先パスを伝えるにとどめる
